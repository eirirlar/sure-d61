...
- [x] **T3: Formal data-format / interface specifications (REQ-1.1, 3.2)** — Ch 6 describes the model chain conceptually but does not provide actual format specs. Core mandate of D6.1 per DoW. Add a structured section (table per interface) to Ch 6.3 covering the four DoW-required interfaces plus the internal pressing→ML interface. Subtasks:
  ...
  - [ ] **T3e: I-6: Pressing FEM CSV → ML model** — PARTIAL, described but field list not in report body. Add the CSV field list (input_hash, 8 geometry/process params, time_to_crack, lip_mean, edge_mean) to Sec 4.3 or 6.3.
- [ ] **T4: Singapore worst-case scenario not named in report body (REQ-8.3)** — Sec 5.2.3 says "representative climate datasets" without naming Singapore. Add the specific boundary condition values (GHI 943 W/m², T_air 28.1°C, T_water 21.5°C, wind 0.5 m/s).
