...
baseline_snapshot: background/eic/2026-09-16_1302_eic_surewave_proposal_v1.md
...
Live text baseline: `background/eic/2026-09-16_1302_eic_surewave_proposal_v1.md` (converted from the docx pulled at 13:02 on submission day). Supersedes the 08:38 snapshot. Note: WP structure changed from 6 to 7 WPs in this snapshot (old WP3 split into new WP3 Verification + new WP4 Performance/Durability; Manufacturing/Deployment/Business all pushed up by one number).

When we pull a fresh snapshot from Teams, save it in `background/eic/` with a new `YYYY-MM-DD_HHMM_` prefix and update the baseline pointer above.
