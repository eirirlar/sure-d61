...
die_box_wall_thickness = 5.
scale = 1.


class RectFloatModel:
    def __init__(self,
                 float_input: RectFloatInput,
                 rect_calc: RectCalc,
                 cup_calc: CupCalc,
                 validate: bool = True
                 ):
    ...
    def calc_float_data(self) -> FloatData:
        # contact = 1e-6 * calc_contact_area(self.rect_calc.inner_side, self.rect_calc.cup_max_x,
        #                                   len(self.rect_calc.cup_points),
        #                                   self.cup_calc.lip_x.circle_r, self.cup_calc.lip_y.circle_r)
        # area = 1e-6 * calc_area(self.float_input.side, self.rect_calc.cup_max_x)
        # w = calc_weight(self.float_input.side, self.rect_calc.cup_max_x, self.float_input.alu_thick)
        # weight = 1e-3 * 2. * w
        # volume_with_cups = calc_inner_vol_plus_cups(self.rect_calc.inner_side, self.float_input.cup_depth,
        #                                            self.float_input, self.rect_calc, self.shoulder_calc)
        # volume_without_cups = calc_volume(self.float_input.cup_depth, self.float_input, self.rect_calc,
        #                                  self.shoulder_calc,
        #                                  self.cup_calc)
        # volume = 1e-6 * (volume_with_cups + volume_without_cups)
        # return FloatData(contact, area, weight, volume)
        return FloatData(0., 0., 0., 0.)

    def calc_fitness(self, nfi: Optional[NormalFloatInput] = None) -> FloatOutput:
        if nfi is None:
            nfi = normalfloatinput.normalize(self.float_input)

        contact_area = calc_contact_area(self.rect_calc.inner_side, self.rect_calc.cup_max_x,
                                         len(self.rect_calc.cup_points),
                                         self.cup_calc.lip_x.circle_r, self.cup_calc.lip_y.circle_r)
        area = calc_area(self.float_input.side, self.rect_calc.cup_max_x)
        weight = calc_weight(self.float_input.side, self.rect_calc.cup_max_x, self.float_input.alu_thick)
        volume = calc_volume(self.float_input.cup_depth, self.float_input, self.rect_calc, self.shoulder_calc,
                             self.cup_calc)
        ...
        structural_integrity = calc_structural_integrity(self.float_input, self.shoulder_calc, self.rect_calc)
...
def create(float_input: RectFloatInput = RectFloatInput(), validate: bool = True) -> RectFloatModel:
    if validate:
        validation_results = float_input.validate()
        if not valid(validation_results):
            raise ValidationError(float_input.__dict__, validation_results)
    # TODO this is just a guesstimate. It depends on the size of the solar panels frame, the cup rad, space, and the lip of the float.
    panel_frame = scale * (float_input.width + float_input.length) / 2. * 0.01
    float_lip = scale * (float_input.width + float_input.length) / 2. * 0.015
    # edge = scale * float_input.cup_rad + scale * float_input.space / 2. + panel_frame + float_lip
    # testing with risen edge
    edge = 28.5
    view_offset_x = -20.
    view_offset_y = 20.
    rect_calc = rectcalc.calc_rect(float_input.width, float_input.length, 2 * float_input.cup_rad, edge,
                                   float_input.space, die_box_wall_thickness, view_offset_x, view_offset_y)
    cup_calc = calc_cup(float_input)
    return RectFloatModel(float_input, rect_calc, cup_calc, validate)


if __name__ == '__main__':
    fi = rectfloatinput.from_fluid_input(fluidio.from_values(
        [4127874840.0,
         28.691915445193025,
         1.0,
         16.80575674442374,
