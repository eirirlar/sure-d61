...
| `generic/` | Cross-cutting / shared | Material that spans SuRE, Gen 2 and Funding (or predates the activity split): company-level correspondence, historical investor updates, legal excerpts, market intel, outgoing customer replies. Contains `background/` (inputs) and `deliverable/` (outputs). |

Commercial/market intel (previously under `commercial/`) is folded into `generic/background/` alongside other cross-cutting material — one-off market snapshots and competitor scans are date-prefixed like other background files. Commercial tasks may still carry the `[COM]` tag.
...
│   ├── background/            – date-prefixed background files + new/ inbox
│   └── deliverable/           – outgoing material (customer replies, quotes, generic company-level deliverables)
...
### T99 `[ ]` [GENERIC] Reply to customer enquiry from X
```
...
| `[GENERIC]` | Cross-cutting / shared (spans multiple activities or predates the split) |
...
- **Single README:** the top-level `README.md` is the *only* README in the repo. Do not create per-folder README files (no `sure/README.md`, no `generic/README.md`, etc.). If a folder's contents need documenting, add or update the description in the *Activity contents* section of the top-level `README.md`.
- **Persistent scripts live in `scripts/`:** Any helper script that will be re-run (batch converters, cleaners, ad-hoc data processors) goes in `scripts/` at the repo root, never inside a data folder like `generic/`, `sure/`, `gen2/`, `funding/` etc. Document each new script in the *Scripts* section of the top-level `README.md` — purpose, how to invoke, inputs/outputs, safety notes. Follows the same pattern as the neighbouring `../fjordgata30` project. Truly single-use one-off commands can stay inline in the conversation without becoming a script.
...
- **Activity tag:** Every new task title must start with an activity tag (`[SURE]`, `[GEN2]`, `[COM]`, `[FUND]`, `[GENERIC]`). If a task spans two activities, pick the primary one; use `[GENERIC]` only when the task is genuinely cross-cutting or lives outside the three main activities. If a new stream is needed, propose an addition to the tag vocabulary and update this document.
...
- **`background/new/` inbox check:** At the start of every working session, check whether `generic/background/new/` (repo-level, cross-cutting inbox) or any `*/background/new/` (activity-level inbox, e.g. `sure/background/new/`, `funding/background/new/`) contains files. If any are present, list them and ask the user whether they should be processed — do not process on your own initiative. Processing means converting to Markdown per the pipeline in the *Background convention* section of `README.md` (`pdftotext -layout` for PDFs, `pandoc … --wrap=none` for DOCX/PPTX/ODT/RTF/HTML, keep pictures as-is), date-prefixing the result, and moving it to the corresponding `background/` folder (`generic/background/` for cross-cutting, activity-level `*/background/` for activity-specific).
- **`background/` naming convention:** Every file in any `background/` folder (`generic/background/` or activity-level `*/background/`) must have a date-prefix of the form `YYYY-MM-DD_short_description.ext`. The date is the document's *own* date (issued / written / sent), not the day it was filed. When you add a new file to a `background/` folder, apply the prefix immediately. When you find an un-prefixed file already there, rename it at next touch and note the rename in the task solution.
